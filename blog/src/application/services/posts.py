...
class PostsService:
    ...
    async def get_posts(self, last_id: str, limit: int = 20) -> PostsResultFromService:
        ...
        post_results.sort(
            key=lambda x: x.post.created_at, reverse=True
        )  # после gather порядок непредсказуем.
        return PostsResultFromService(
            posts=post_results, has_next=has_next, last_id=last_id
        )
